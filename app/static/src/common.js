...
window.showLoading = function (duration = 4000) {
  if (loader && loader.style.display !== "flex") {
    ...
    if (duration !== Infinity) {
      setTimeout(window.hideLoading, duration);
    }
  }
};
...
function setupPageFeatures() {
  function updateMenu() {
    ...
    for (let item of items) {
      ...
      item.classList.toggle("active", currentUrl.includes(keyword));
    }
  }
  ...
  function setupPlaylistToggle() {
    ...
    if (!playlistNav || !playlistsList) return;

    const icon = playlistNav.querySelector(".playlist-toggle-icon");
