...
const seedsModule = (() => {
  ...
  const addToSeeds = (item, type) => {
    ...
    seedElement.innerHTML = `<div class="seed-info"><a href="${spotifyUrl}"target="_blank"rel="noopener noreferrer"class="seed-name"title="${item.name}">${item.name}</a><div class="seed-subtext">${type === "track" ? item.artists[0].name : "Artist"}</div></div><button class="remove-seed"aria-label="Remove seed">×</button>`;
    seedElement
    ...
    const createResultElement = (item, type) => {
      ...
      const spotifyUrl =
        type === "track"
          ...
          : item.external_urls.spotify;
      element.innerHTML = `<img src="${imageUrl || "/static/dist/img/default-track.svg"}"alt="${item.name}"class="result-image"><div class="result-info"><div class="result-name"title="${item.name}">${item.name}</div><div class="result-subtext">${type === "track" ? item.artists[0].name : "Artist"}</div><div class="spotify-button"aria-label="Listen on Spotify"><i class="rawcon-spotify"onclick="window.open('${spotifyUrl}', '_blank')"></i><span class="spotify-text">Listen on Spotify</span></div></div><div class="result-actions"><button class="add-to-seeds"data-id="${item.id}"data-type="${type}"data-tooltip="Add to seeds">+</button></div>`;
    ...
    try {
      ...
      if (playlists.error) {
        ...
        const playlistsHtml = playlists
          .map(
            (playlist) =>
              `<div class="playlist-choice"data-playlistid="${playlist.id}"><div class="playlist-div"><img src="${playlist.cover_art}"alt="${playlist.name}"class="playlist-art"><h3 class="playlist-title"title="${playlist.name}">${playlist.name}</h3></div></div>`,
          )
          .join("");
